...
import {
  X, CreditCard, Banknote, Clock, Utensils, ShoppingBag, Car,
  Minus, Plus, UserPlus, Tag, FileText, CheckCircle, AlertCircle, Loader2,
} from 'lucide-react';
...
export default function CheckoutModal() {
  ...
  const handleOrderTypeChange = (type: 'takeout' | 'drive-through' | 'dine-in') => {
    setOrderType(type);
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!selectedCustomer || !selectedCustomer.name || !selectedCustomer.name.trim()) {
      toast.error('Please add customer details before placing the order.');
      return;
    }
    if (paymentTiming === 'pay-now') {
      if (paymentType === 'split') {
        if (splitPayments.length < 2) {
          toast.error('Add at least 2 payment entries for split payment.');
          return;
        }
        for (let i = 0; i < splitPayments.length; i++) {
          const sp = splitPayments[i];
          if (!sp.amount || sp.amount <= 0) {
            toast.error(`Please enter a valid amount for Person ${i + 1}.`);
            return;
          }
        }
        const sumOk = Math.abs(splitTotal - total) < 0.01;
        if (!sumOk) {
          toast.error(`Split amounts ($${splitTotal.toFixed(2)}) don't match total ($${total.toFixed(2)}).`);
          return;
        }
      } else {
    ...
    if (order) {
      toast.success(
        <div className="flex flex-col gap-0.5 text-left">
          <span className="font-700 text-[11px] text-green-900">Order Placed! 🎉</span>
          <span className="text-[10px] text-green-800">{order.orderNumber} • {order.orderType.toUpperCase()}</span>
          <span className="font-600 text-[10px] text-green-950">
            {order.paymentStatus === 'unpaid' ? '⏳ Payment Pending' : `✓ Paid $${order.total.toFixed(2)}`}
          </span>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-[2px] flex items-center justify-center p-3">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden animate-scale-up">
          ...
          <div className="bg-brand-primary px-5 py-3 flex items-center gap-3 flex-shrink-0">
            <h2 className="text-[14px] font-800 text-white uppercase tracking-widest">Your Order</h2>
          ...
          <div className="flex flex-1 overflow-hidden min-h-0">
            ...
            <div className="w-1/2 border-r border-neutral-100 flex flex-col overflow-hidden">
              ...
              {paymentTiming === 'pay-later' && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-[11px] font-700 text-amber-800">Payment Pending</p>
                    <p className="text-[10px] text-amber-600 mt-0.5">
                      Order will be placed. Customer can pay when picking up (within 1–3 hours).
                    </p>
                  </div>
                </div>
              )}
              ...
              {paymentTiming === 'pay-now' && paymentType === 'one-time' && (
                <>
                  {/* Amount */}
                  <div>
                    <label className="block text-[10px] font-600 text-neutral-500 mb-1.5 uppercase tracking-wide">Amount To Pay ($)</label>
                    <div className="border border-neutral-200 rounded-xl px-3.5 py-2.5 bg-brand-primary/5">
                      <span className="text-[16px] font-800 text-brand-primary">${total.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  {paymentMethod === 'cash' && (
                    <div className="space-y-2">
                      <p className="text-[10px] font-600 text-neutral-500 uppercase tracking-wide">Cash Denominations</p>
                      <div className="grid grid-cols-3 gap-1.5">
                        {DENOMINATIONS.map((denom) => {
                          const qty = cashDenominations[denom] || 0;
                          return (
                            <div key={denom} className="bg-neutral-50 border border-neutral-200 rounded-xl p-2 flex flex-col items-center gap-1.5">
                              <div className="flex items-center justify-between w-full">
                                <span className="text-[10px] font-700 text-neutral-700">${denom}</span>
                                <button
                                  onClick={() => setCashDenomination(denom, qty - 1)}
                              ...
                              <div className="flex items-center justify-between w-full">
                                <span className="text-[13px] font-800 text-neutral-900 w-6 text-center">{qty}</span>
                                <button
                                  onClick={() => setCashDenomination(denom, qty + 1)}
                      ...
                      <div>
                        <label className="block text-[10px] font-600 text-neutral-500 mb-1.5 uppercase tracking-wide">Or Enter Custom Amount</label>
                        <div className="relative">
                          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[12px] font-500 text-neutral-400">$</span>
                          <input
                            type="number"
                            placeholder="0.00"
                            min="0"
                            step="0.01"
                            value={cashGiven > 0 && Object.values(cashDenominations).every(q => q === 0) ? cashGiven : ''}
                      ...
                      {cashGiven > 0 && (
                        <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-3 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-neutral-500 font-500">Cash Given</span>
                            <span className="text-[11px] font-700 text-neutral-800">${cashGiven.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-neutral-500 font-500">Change Due</span>
                            <span className={`text-[12px] font-800 ${changeAmount > 0 ? 'text-green-600' : 'text-neutral-400'}`}>
              ...
              {paymentTiming === 'pay-now' && paymentType === 'split' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-600 text-neutral-500 uppercase tracking-wide">Split Entries</p>
                    <span className={`text-[10px] font-700 ${Math.abs(splitTotal - total) < 0.01 ? 'text-green-600' : 'text-amber-600'}`}>
                      ${splitTotal.toFixed(2)} / ${total.toFixed(2)}
                    </span>
                  </div>

                  {splitPayments.map((sp, i) => (
                    <div key={i} className="bg-neutral-50 border border-neutral-200 rounded-xl p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-700 text-neutral-600">Person {i + 1}</span>
                        <button
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          value={sp.personName || ''}
                          onChange={(e) => updateSplitPayment(i, { ...sp, personName: e.target.value })}
                          className="border border-neutral-200 rounded-lg px-2.5 py-1.5 text-[10px] font-500 focus:outline-none focus:border-brand-primary bg-white"
                        />
                        <input
                          type="number"
                          placeholder="Amount"
                          ...
                          step="0.01"
                          value={sp.amount || ''}
                          onChange={(e) => updateSplitPayment(i, { ...sp, amount: parseFloat(e.target.value) || 0 })}
                          className="border border-neutral-200 rounded-lg px-2.5 py-1.5 text-[10px] font-500 focus:outline-none focus:border-brand-primary bg-white"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-1">
                        {(['cash', 'card'] as const).map((m) => (
                          <button
                            key={m}
                            onClick={() => updateSplitPayment(i, { ...sp, method: m })}
                            className={`py-1.5 rounded-lg text-[9px] font-600 uppercase transition-all cursor-pointer ${
                              sp.method === m
                  ...
                  {/* Remaining hint */}
                  {splitRemaining > 0.01 && (
                    <p className="text-[10px] text-amber-600 font-500 flex items-center gap-1">
                      <AlertCircle size={10} />
                  ...
                  <button
                    onClick={() => addSplitPayment({ method: 'cash', amount: splitRemaining > 0 ? parseFloat(splitRemaining.toFixed(2)) : 0 })}
                    className="w-full py-2 border border-dashed border-brand-primary text-brand-primary rounded-xl text-[10px] font-600 hover:bg-orange-50 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <Plus size={11} /> Add Split Entry
                  </button>
                </div>
              )}
            ...
            <div className="w-1/2 flex flex-col overflow-y-auto p-4 space-y-3 no-scrollbar">
              ...
              {orderTiming === 'later' && scheduledAt && (
                <div className="bg-orange-50 border border-orange-100 rounded-xl p-3 flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] font-600 text-brand-primary">Scheduled Pickup</p>
                    <p className="text-[11px] font-700 text-neutral-800">{formatScheduled(scheduledAt)}</p>
                  </div>
                  <button
              ...
              <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 space-y-2">
                <p className="text-[11px] font-700 text-neutral-800 uppercase tracking-wide text-center mb-3">Order Summary</p>

                {/* Items count */}
                <div className="flex items-center justify-between pb-2 border-b border-neutral-100">
                  <span className="text-[10px] text-neutral-500 font-500">{cartItems.length} item{cartItems.length !== 1 ? 's' : ''}</span>
                  <span className="text-[10px] font-600 text-neutral-700">${subtotal.toFixed(2)}</span>
                </div>

                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-neutral-500 font-500">Sub Total</span>
                    <span className="text-[10px] font-600 text-neutral-700">${subtotal.toFixed(2)}</span>
                  </div>
                  {discount > 0 && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-600 text-green-600">-${discount.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-neutral-500 font-500">GST (5%)</span>
                    <span className="text-[10px] font-600 text-neutral-700">${tax.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-neutral-500 font-500">Total Tax (5%)</span>
                    <span className="text-[10px] font-600 text-neutral-700">${tax.toFixed(2)}</span>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-neutral-200 mt-1">
                  <span className="text-[11px] font-700 text-neutral-900">Amount To Pay</span>
                  <span className="text-[15px] font-800 text-brand-primary">${total.toFixed(2)}</span>
                </div>
                ...
                {paymentTiming === 'pay-later' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 flex items-center gap-1.5 mt-1">
                    ...
                    <span className="text-[10px] font-600 text-amber-700">Will be marked as UNPAID until collected</span>
                  </div>
                )}
              </div>
      ...
      <OrderLaterModal
        ...
        onConfirm={(iso) => { setScheduledAt(iso); setOrderTiming('later'); }}
      />
      <PromoDiscountModal
        isOpen={showPromo}
