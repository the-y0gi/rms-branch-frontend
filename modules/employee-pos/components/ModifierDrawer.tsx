...
export default function ModifierDrawer({ item, isOpen, onClose }: Props) {
  ...
  const livePrice = () => {
    const modSum = Object.values(selections)
      .flat()
            ...
            {item.modifierGroups?.map((g, i) => {
              ...
              return (
                ...
                >
                  ...
                  {count > 0 ? (
                    <span
                      className={`text-[8px] px-1.5 py-0.5 rounded-full font-700 ${active ? "bg-white/25 text-white" : "bg-brand-primary/10 text-brand-primary"}`}
                    >
                      {count}
                    </span>
                  ) : (
            ...
            {activeGroup && (
              <>
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-neutral-100">
                  <h4 className="text-[10px] font-700 text-neutral-700 uppercase tracking-wide">
                    {activeGroup.name}
                  </h4>
                  <span className="text-[9px] font-500 text-neutral-400">
                    {activeGroup.maxSelection === 1
                      ? "Select 1"
                      : `Up to ${activeGroup.maxSelection}`}
                  </span>
                </div>
                  ...
                  {activeGroup.options.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`absolute top-2.5 right-2.5 w-4 h-4 rounded-full border flex items-center justify-center transition-all ${sel ? "bg-brand-primary border-brand-primary" : "border-neutral-300"}`}
                        >
                          {sel && (
                            <Check
                              size={9}
